class User < ActiveRecord::Base
  ...
  def send_email_alerts_before_paycheck
    dates_before_payments = self.payments.all.map{|p| p.transaction_dates}.flatten.map{|b| b-8}
    dates_before_deposits = self.deposits.all.map{|d| d.deposit_dates}.flatten.map{|c| c-8}
