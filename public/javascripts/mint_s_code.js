...
wa.lastUpdate="20110224";
if(document.getElementById('buildNumber')){ wa.buildNum=document.getElementById('buildNumber').innerHTML.toLowerCase(); }else{ wa.buildNum="build undefined"; }

/************************** START CONFIG SECTION **************************/
wa.reportSuiteQA="intuitmintqa"
wa.reportSuiteProd="intuitmintprod"
wa.internalDomains="data.mint.com,www.mint.com,wwws.mint.com,forums.mint.com,maintenance.mint.com,blog.mint.com,satisfaction.mint.com,getsatisfaction.com,fool.mint.com,answers.mint.com";  //comma seperate list
wa.defaultSiteName="mint";
wa.defaultSiteGroup="mktg";
wa.bu=""; //optional if site aligns with larger business unit
/************************** END CONFIG SECTION **************************/

//Domain Logic For QA/Prod Report Suite
var hostname = window.location.hostname;
var s_account=wa.reportSuiteQA //default
...
s.linkTrackVars="prop3,eVar3,prop4,eVar4,eVar7,eVar9,eVar10,eVar12,prop26,eVar32,eVar37,eVar43,eVar50,prop60,eVar60,prop61,eVar61,state,zip"
...
switch(wa.pageDetail)
{
	...
	case "overview":
		...
		if(!isVarEmpty(document.getElementById("turbotax-welcome"))) {
			wa.pageState="turbotax-welcome";
			if(!isVarEmpty(wa.userGuid)) {s.events=s.events+",event7:"+wa.userGuid;}
		}
		break;
...
//	case "ira":
//		s.events="event21";  //first step of ira
//		break;
	...
	switch(pv_details) {
		...
		case "quiz":	
			...
			switch(pv_action) {
				case "filing status":
				case "quiz":
					s.events="event23"; //IRA Filing Status (first question in both versions of quiz)
 ...
 */
try{
	$('[class*=launch][class*=vid]').click(function(){	
...
function s_doPlugins(s) {
	...
	// The source is pulled from a query string parameter.
	...
	if(cid) {	
		...
		//s.prop25=s.campaign;if(!isVarEmpty(s.prop25)) { s.eVar25="D=c25"; }
    ...
    */
	...
	if(wa.pageDetail=="overview"&& !isVarEmpty(wa.pageState)) {
		...
		s.prop7=wa.msgId= "msg"+s.getQueryParam('messageId','',curl);  
        ...
        case "ty10":
			...
			if(pv_linkDestination) {
				...
				if(pv_linkDestination!="finished" && pv_linkDestination!="taxcaster_lnk" && pv_linkDestination!="taxcaster_btn" && pv_linkDestination!="get started" && pv_linkDestination!="learn more") {
					s.prop21="mint_overview_ty10widget_"+pv_details+"_"+pv_linkDestination;
...
function waOfferClick(obj,offerId,offerSource,campaign,displayModule)
{
    ...
    switch(wa.pageDetail) {		
		...
		//	if(!isVarEmpty(campaign)){s.prop6=s.prop6+"_"+campaign;}
		//	s.eVar6="D=c6";
...
function waGlobalNav(pv_location,pv_sectionHeader,pv_linkText,pv_queryParam,pv_creative,pv_pageNameOverride) {
	...
	waLinkClick(this,'globalnav','link');
...
function waTransactionBuildPageName(pv_details) {
	...
	//if(pv_details) s.pageName=s.pageName?s.pageName+"/"+pv_details:pv_details;
...
function waPlanningActions(pv_action,pv_details) {
	...
	if(makeCall=="true") {
		waLinkClick(this,"planning",callType); //send call
	}
	s.prop5=s.prop7="";
}
...
function waInvestmentActions(pv_action,pv_details) {
	var callType="link";
	var prefix="invest:";
	...
	switch(pv_action) {
		case "alertimpression":	
			if(pv_alertIDs.length>0) {
				...
				wa.events=s.events="prodview,event35";
...
function waAdviceActions(pv_action,pv_adviceIDs,pv_details) {	
	var advicePage=waBuildAdvicePage();
/*
	//REPLACE WITH waBuildAdvicePage()
	if(wa.containerPageName) {
		advicePage=wa.containerPageDetail;
	} else {
		advicePage=wa.pageDetail;
	}
*/	
	...
	switch(pv_action) {
		case "impression":	wa.events=s.events="prodview,event42";	//event42=advice impression
			break;
